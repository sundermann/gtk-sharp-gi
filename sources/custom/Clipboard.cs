...
namespace Gtk {
	...
	public partial class Clipboard {
		...
		public bool WaitForTargets(out Gdk.Atom[] targets) {
			...
			bool ret = gtk_clipboard_wait_for_targets(Handle, arrayPtr, out count);

			targets = new Gdk.Atom [count];
			for (int i = 0; i < count; i++)
				targets [i] = new Gdk.Atom(Marshal.ReadIntPtr ((IntPtr)((long)arrayPtr + i * IntPtr.Size)));
			GLib.Marshaller.Free (arrayPtr);
			return ret;
		}
	}
}
